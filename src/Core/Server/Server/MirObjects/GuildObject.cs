...
namespace Server.MirObjects
{
    public sealed class GuildObject
    {
        ...
        public void PlayerLogged(PlayerObject member, bool online, bool New = false)
        {
            ...
            SendServerPacket(new S.GuildMemberChange() {Name = member.Name, Status = (byte)(New? 2: online? 1: 0)});
        ...
        public void SendGuildStatus(PlayerObject member)
        {
            ...
            member.Enqueue(new S.GuildStatus()
                {
                    GuildName = gName,
        ...
        public bool ChangeRank(PlayerObject self, string memberName, byte rankIndex, string rankName = "Members")
        {
            ...
            if (player != null)
            {
                ...
                player.Enqueue(new S.GuildMemberChange() { Name = self.Info.Name, Status = (byte)8, Ranks = NewRankList });
            ...
            for (int i = 0; i < Ranks.Count; i++)
                for (int j = 0; j < Ranks[i].Members.Count; j++)
                    if ((Ranks[i].Members[j].Player != null) && (Ranks[i].Members[j].Player != Member.Player))
                    {
                        ...
                        player.Enqueue(new S.GuildMemberChange() { Name = Member.Name, Status = (byte)5, RankIndex = (byte)MemberRank.Index });
        ...
        public bool NewRank(PlayerObject Self)
        {
            ...
            SendServerPacket(new S.GuildMemberChange() { Name = Self.Name, Status = (byte)6, Ranks = NewRankList});
            NeedSave = true;
            return true;
        }
        ...
        public bool ChangeRankOption(PlayerObject Self, byte RankIndex, int Option, string Enabled)
        {
            ...
            SendServerPacket(new S.GuildMemberChange() { Name = Self.Name, Status = (byte)7, Ranks = NewRankList });
            NeedSave = true;
            return true;
        }
        public bool ChangeRankName(PlayerObject Self, string RankName, byte RankIndex)
        {
            ...
            for (int i = 0; i < Ranks.Count; i++)
            {
                for (int j = 0; j < Ranks[i].Members.Count; j++)
                {
                    ...
                    if (player != null)
                    {
                        player.Enqueue(new S.GuildMemberChange() { Name = Self.Info.Name, Status = (byte)7, Ranks = NewRankList });
                        player.GuildMembersChanged = true;
                        if (i == RankIndex)
                        {
                            player.BroadcastInfo();
        ...
        public void MemberDeleted(string name, PlayerObject formerMember, bool kickSelf)
        {
            for (int i = 0; i < Ranks.Count; i++)
            {
                for (int j = 0; j < Ranks[i].Members.Count; j++)
                {
                    ...
                    {
                        PlayerObject player = (PlayerObject)Ranks[i].Members[j].Player;
                        player.Enqueue(new S.GuildMemberChange() { Name = name, Status = (byte)(kickSelf ? 4 : 3) });
                        player.GuildMembersChanged = true;
                    }
                }
            }
            ...
            if (formerMember != null)
            {
                ...
                formerMember.Enqueue(new S.GuildStatus() { GuildName = "", GuildRankName = "", MyOptions = (GuildRankOptions)0 });
        ...
        public void NewNotice(List<string> notice)
        {
            ...
            SendServerPacket(new S.GuildNoticeChange() { update = -1 });
        }
        ...
        public void GainExp(uint amount)
        {
            ...
            else
            {
                ...
                {
                    NextExpUpdate = Envir.Time + 10000;
                    SendServerPacket(new S.GuildExpGain() { Amount = expAmount });
                }
            }
        }
            ...
            for (int i = 0; i < Ranks.Count; i++)
            {
                for (int j = 0; j < Ranks[i].Members.Count; j++)
                {
                    ...
                    if (player != null)
                    {
                        ...
                        player.Enqueue(new S.ColourChanged { NameColour = player.GetNameColour(player) });
                        player.BroadcastInfo();
                    }
                }
            }
        ...
        public void Process()
        {
            ...
            for (int k = 0; k < BuffList.Count; k++)
            {
                ...
                if (BuffList[k].ActiveTimeRemaining < 0)
                {
                    ...
                    //SendServerPacket(new S.RemoveGuildBuff {ObjectID = (uint)BuffList[k].Id});
                }
            }
            ...
            if (NeedUpdate)
            {
                ...
                {
                    SendServerPacket(new S.GuildBuffList { ActiveBuffs = UpdatedBuffs });
                }
        ...
        public void NewBuff(int Id, bool charge = true)
        {
            ...
            SendServerPacket(new S.GuildBuffList { ActiveBuffs = NewBuff });
        ...
        public void ActivateBuff(int Id)
        {
            ...
            SendServerPacket(new S.GuildBuffList { ActiveBuffs = NewBuff });
            SendServerPacket(new S.GuildStorageGoldChange() { Type = 2, Name = "", Amount = (uint)buff.Info.ActivationCost });
        ...
        public void RemoveAllBuffs()
        {
            ...
            SendServerPacket(new S.GuildBuffList {Remove = 1, ActiveBuffs = BuffList});
