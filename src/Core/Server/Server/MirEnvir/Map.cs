        ...
        private void CompleteMagic(IList<object> data)
        {
            ...
            switch (magic.Spell)
            {
                ...
                case Spell.HolyTrinity:
                    ...
                    {
                        case Spell.HolyTrinity:
                            type = BuffType.HolyTrinity;
                            break;
                        case Spell.SoulShield:
                            type = BuffType.SoulShield;
                            break;
                        default:
                            type = BuffType.BlessedArmour;
                            break;
                    }
                                ...
                                switch (target.Race)
                                {
                                    ...
                                    case ObjectType.Hero:
                                        ...
                                        if (target.IsFriendlyTarget(player))
                                        {
                                            ...
                                            var stats = new Stats { Values = new SortedDictionary<Stat, int>() };

                                            switch (type)
                                            {
                                                case BuffType.HolyTrinity:
                                                    stats.Values.Add(Stat.MaxAC, target.Level / 7 + 4);
                                                    stats.Values.Add(Stat.MaxMAC, target.Level / 7 + 4);
                                                    switch (((HumanObject)target).Class)
                                                    {
                                                        case MirClass.Taoist:
                                                            stats.Values.Add(Stat.MaxSC, value);
                                                            break;
                                                        case MirClass.Assassin:
                                                        case MirClass.Warrior:
                                                            stats.Values.Add(Stat.MaxDC, value);
                                                            break;
                                                        case MirClass.Archer:
                                                        case MirClass.Wizard:
                                                            stats.Values.Add(Stat.MaxMC, value);
                                                            break;
                                                    }
                                                    break;
