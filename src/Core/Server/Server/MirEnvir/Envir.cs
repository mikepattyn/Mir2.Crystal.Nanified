...
using C = Shared.Packets.Client.Models;
using ObjectType = Shared.Enums.ObjectType;
using S = Shared.Packets.Server.Models;

namespace Server.MirEnvir;
public class Mir2Envir
{
    public Mir2Envir() 
    {
        Main = this;
    }
    public static Mir2Envir Main;
...
{
    public static void BroadcastNotification(this Mir2Envir mir2Envir, string Message)
    {
...
public class Envir
{
    private PacketService PacketService { get; set; }

    public Envir(PacketService packetService)
    {
    ...
    private void Connection(IAsyncResult result)
    {
        ...
        try
        {
            ...
            lock (Connections)
                Connections.Add(new MirConnection(++_sessionID, tempTcpClient, PacketService));
        }
        catch (Exception ex)
        {
            MessageQueue.Enqueue(ex);
    ...
    public void NewAccount(C.NewAccount p, MirConnection c)
    {
        ...
        lock (AccountLock)
        {
            ...
            {
                c.Enqueue(new S.NewAccount { Result = 7 });
                return;
            }
            ...
            c.Enqueue(new S.NewAccount { Result = 8 });
        }
    }

    public int HTTPNewAccount(C.NewAccount p, string ip)
    {
        if (!Settings.AllowNewAccount)
        {
            return 0;
    ...
    public void ChangePassword(C.ChangePassword p, MirConnection c)
    {
        if (!Settings.AllowChangePassword)
        {
            c.Enqueue(new S.ChangePassword { Result = 0 });
            return;
        }

        if (!AccountIDReg.IsMatch(p.AccountID))
        {
            c.Enqueue(new S.ChangePassword { Result = 1 });
            return;
        }

        if (!PasswordReg.IsMatch(p.CurrentPassword))
        {
            c.Enqueue(new S.ChangePassword { Result = 2 });
            return;
        }

        if (!PasswordReg.IsMatch(p.NewPassword))
        {
            c.Enqueue(new S.ChangePassword { Result = 3 });
            return;
        }
        ...
        if (account == null)
        {
            c.Enqueue(new S.ChangePassword { Result = 4 });
            return;
        }
        ...
        if (account.Banned)
        {
            ...
            {
                c.Enqueue(new S.ChangePasswordBanned { Reason = account.BanReason, ExpiryDate = account.ExpiryDate });
                return;
            }
        ...
        p.CurrentPassword = Crypto.HashPassword(p.CurrentPassword, account.Salt);
        if (string.CompareOrdinal(account.Password, p.CurrentPassword) != 0)
        {
            c.Enqueue(new S.ChangePassword { Result = 5 });
            return;
        }

        account.Password = p.NewPassword;
        account.RequirePasswordChange = false;
        c.Enqueue(new S.ChangePassword { Result = 6 });
    }
    public void Login(C.Login p, MirConnection c)
    {
        if (!Settings.AllowLogin)
        {
            c.Enqueue(new S.Login { Result = 0 });
            return;
        }

        if (!AccountIDReg.IsMatch(p.AccountID))
        {
            c.Enqueue(new S.Login { Result = 1 });
            return;
        }

        if (!PasswordReg.IsMatch(p.Password))
        {
            c.Enqueue(new S.Login { Result = 2 });
            return;
        }
        var account = GetAccount(p.AccountID);
        ...
        if (account == null)
        {
            c.Enqueue(new S.Login { Result = 3 });
            return;
        }
        ...
        p.Password = Crypto.HashPassword(p.Password, account.Salt);
        ...
        if (string.CompareOrdinal(account.Password, p.Password) != 0)
        {
            ...
            c.Enqueue(new S.Login { Result = 4 });
            return;
        }
        ...
        {
            c.Enqueue(new S.Login { Result = 5 });
            return;
        }
        ...
        c.Enqueue(new S.LoginSuccess { Characters = account.GetSelectInfo() });
    }
    ...
    public void NewCharacter(C.NewCharacter p, MirConnection c, bool IsGm)
    {
        if (!Settings.AllowNewCharacter)
        {
            c.Enqueue(new S.NewCharacter { Result = 0 });
            return;
        }

        if (!CharacterReg.IsMatch(p.Name))
        {
            c.Enqueue(new S.NewCharacter { Result = 1 });
            return;
        }
        ...
        {
            c.Enqueue(new S.NewCharacter { Result = 2 });
            return;
        }
        ...
        {
            c.Enqueue(new S.NewCharacter { Result = 3 });
            return;
        }
        ...
        for (var i = 0; i < c.Account.Characters.Count; i++)
        {
            ...
            {
                c.Enqueue(new S.NewCharacter { Result = 4 });
                return;
            }
        }
        ...
        lock (AccountLock)
        {
            ...
            {
                c.Enqueue(new S.NewCharacter { Result = 5 });
                return;
            }
            ...
            c.Enqueue(new S.NewCharacterSuccess { CharInfo = info.ToSelectInfo() });
        }
    }

    public bool CanCreateHero(C.NewHero p, MirConnection c, bool IsGm)
    {
