...
namespace Server.MirEnvir
{
    public class Dragon
    {
        ...
        protected bool DropItem(UserItem item)
        {
            ...
            ItemObject ob = new ItemObject(LinkedMonster, item, droplocation)
            {
                Owner = LinkedMonster.EXPOwner,
        ...
        {
            if (LinkedMonster.EXPOwner != null && LinkedMonster.EXPOwner.CanGainGold(gold))
            {
                LinkedMonster.EXPOwner.WinGold(gold);
                return true;
            }

            Point droplocation = new Point(DropArea.Left + DropArea.Width / 2, DropArea.Top);
            ItemObject ob = new ItemObject(LinkedMonster, gold, droplocation)
            {
                Owner = LinkedMonster.EXPOwner,
                OwnerTime = Envir.Time + Settings.Minute,
            };
