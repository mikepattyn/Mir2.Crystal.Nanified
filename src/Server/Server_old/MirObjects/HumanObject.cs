...
using Server.MirObjects.Player;
using Shared;
using Shared.Enums;
...
namespace Server.MirObjects
{
    public class HumanObject : MapObject
    {
                ...
                if (poison.Owner != null)
                {
                    switch (poison.Owner.Race)
                    {
                        case ObjectType.Player:
                            ...
                            DelayedAction action = new DelayedAction(DelayedType.Magic, Envir.Time, poison.Owner, caster.GetMagic(Spell.DelayedExplosion), poison.Value, CurrentLocation);
        ...
        public void Magic(Spell spell, MirDirection dir, uint targetID, Point location)
        {
            ...
            switch (spell)
            {
                ...
                case Spell.HolyTrinity:
                    HolyTrinity(magic, target == null ? location : target.CurrentLocation, out cast);
                    break;
                default:
                    cast = false;
                    break;
        ...
        private void HolyTrinity(UserMagic magic, Point location, out bool cast)
        {
            cast = false;
            UserItem item = GetAmulet(1);
            if (item == null) return;
                ...
                case Spell.FireBounce:
                    ...
                    if (target.Race == ObjectType.Monster)
                    {
                        ...
                        if (targets.Count > 0)
                        {
                            ...
                            FireBounce(nextTarget, magic, target, --bounce);
                        }
                    }
                    ...
                    for (int y = place.Y - 1; y <= place.Y + 1; y++)
                    {
                        ...
                        for (int x = place.X - 1; x <= place.X + 1; x++)
                        {
                            ...
                            for (int i = 0; i < cell.Objects.Count; i++)
                            {
                                ...
                                {
                                    case ObjectType.Monster:
                                        if (targetob == null || !targetob.IsAttackTarget(this) || targetob.Node == null || targetob.Level > Level + 2) continue;
